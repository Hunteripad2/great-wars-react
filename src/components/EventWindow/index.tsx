...
const EventWindow = observer(() => {
	...
	function chooseSecondEventOption(eventData : Event) {
		if (eventData.type === "music") {
			...
			if (!newMusicList.some(track => track.name === eventData.newMusicName)) {
				...
				state.closeTabs();
			} else alert("Эта композиция уже находится в списке");
		}
	}
	...
	return (
		<div className={styles.eventWindow} style={state.eventWindowStyle}>
			...
			<button className={styles.optionFirst} onClick={() => chooseFirstEventOption(currentEventData)} style={state.eventFirstOptionStyle}>{currentEventData.option1}</button>
			{currentEventData.option2 ? <button className={styles.optionSecond} onClick={() => chooseSecondEventOption(currentEventData)}>{currentEventData.option2}</button> : null}
		</div>
	);
});
